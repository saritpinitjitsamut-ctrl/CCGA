...
function renderOrderCard(o) {
    ...
    if (status === STATUS_PENDING) {
        ...
        actionsHtml = `
            ...
            ${canFinish ? (() => {
                const hasUndispatched = ordersWithUndispatchedMaterials.has(o.id);
                const btnStyle = hasUndispatched
                    ? `background:rgba(245,158,11,0.15);border:1px solid rgba(245,158,11,0.4);color:#fbbf24;`
                    : ``;
                const btnTitle = hasUndispatched ? `title="⚠️ ยังไม่จ่ายวัสดุ — กรุณาจ่ายวัสดุในหน้าคลังวัสดุก่อน"` : ``;
                const btnLabel = hasUndispatched ? `⚠️ ผลิตสำเร็จ` : `✅ ผลิตสำเร็จ`;
