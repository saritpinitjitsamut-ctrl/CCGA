...
async function generateProductionNumber(orderDateValue, existingValue = null) {
    ...
    if (!dbSupabase) return `${prefix}001`;
...
async function generateProductionNumberForUpload(orderDateValue, existingValue = null) {
    ...
    const prefix = result.replace(/\/[0-9]+$/, '/');
    const currentSeq = parseInt(result.split('/').pop() || '0', 10);
    if (!productionNumberUploadCache[prefix] || productionNumberUploadCache[prefix] <= currentSeq) {
        ...
        if (bulkUploadPMBtn) {
            bulkUploadPMBtn.addEventListener('click', async () => {
                ...
                pmRows.forEach(row => {
                    ...
                    const key = `${String(row.order_number||'').trim()}|${String(row.product_code||'').trim()}`;
                    const match = byOrderSku.get(key);
                    if (match && match.payment_time) {
                        try {
                            const a = new Date(match.payment_time);
                            const b = new Date(row.payment_time);
                            if (!isNaN(a.getTime()) && !isNaN(b.getTime())) {
                                const diffMs = Math.abs(a.getTime() - b.getTime());
                                const oneHour = 3600000;
                                const sameDay = (a.toISOString().split('T')[0] === b.toISOString().split('T')[0]);
                                if (sameDay || diffMs <= oneHour) { skipped++; return; }
                            }
                        } catch (e) {
                            // if compare fails, proceed to insert
                        }
                    }
                    toInsert.push(row);
                });
