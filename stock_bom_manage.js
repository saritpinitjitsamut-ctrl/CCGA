...
function renderBomRows() {
    if (!bomRowsLoaded || !skuMasterRowsLoaded) {
    ...
    const productsWithBom = new Set(bomRows.map(row => normalizeCode(row.product_code)));
    ...
    bomCount.textContent = `${filtered.length + masterOnlyRows.length} รายการ`;
    if (!filtered.length && !masterOnlyRows.length) {
        bomTableBody.innerHTML = '<tr><td colspan="5" class="empty-state">ไม่พบ BOM</td></tr>';
        return;
    }

    const bomHtml = filtered.map((row, index) => {
    ...
    const masterHtml = masterOnlyRows.map((item, index) => `
        <tr data-master-code="${item.product_code}" class="master-only-row">
            <td class="sequence-column">${filtered.length + index + 1}</td>
    ...
    bomTableBody.innerHTML = bomHtml + masterHtml;
}
...
async function refreshAllData() {
    ...
    skuMasterRowsLoaded = false;
    renderBomRows();
    await Promise.all([loadBomRows(), loadSkuMasterRows()]);
    isRefreshing = false;
    renderBomRows();
}
...
function openSkuFromGlobalSearch(code) {
    const existingBom = bomRows.find(row => normalizeCode(row.product_code) === normalizeCode(code));
    bomSearchInput.value = code;
    appliedSearchTerm = String(code || '').trim();

    if (existingBom) {
        setBomForm(existingBom);
...
function handleTableClick(event) {
    ...
    if (masterRow) {
        openSkuFromGlobalSearch(masterRow.dataset.masterCode);
        return;
    }
...
async function initializePage() {
    ...
    if (bomStatusFilter) bomStatusFilter.addEventListener('change', renderBomRows);
    // addComponent button
